...
def run():                  #Initial function asking the user what they intend to do.
    print('Do you wish to move your character? You are currently located at location : [%d, %d]' %(vertical, horizontal))           #Asks if the user wishes to move and prints out their current location
    ...
    while direction_check < 1 or direction_check > 5:       #While user input is integer, but outside of the acceptable range
        ...
        if direction_check == 1:                            #User input = 1
            if vertical < 1:                                #Setting a limit for how far a user can move North.
                ...
                print('You have moved North. You are now at location : [%d, %d]' %(vertical, horizontal))   #Tells the user their new location
                movement()                                  #Re-run movement function
            else:                                           #If user is at the Northern limit
                print('You cannot go any further North. You are already located at location : [%d, %d]' %(vertical, horizontal))    #Tells user they can't go any further North and reminds them of their location
                movement()                                  #Re-run movement function
        elif direction_check == 2:                          #Repeat for user input = 2 (East)
            if horizontal < 1:
                horizontal += 1
                print('You have moved East. You are now at location : [%d, %d]' %(vertical, horizontal))
                movement()
            else:
            ...
            if vertical > -1:
                vertical -= 1
                print('You have moved South. You are now at location : [%d, %d]' %(vertical, horizontal))
                movement()
            else:
            ...
            if horizontal > -1:
                horizontal -= 1
                print('You have moved West. You are now at location : [%d, %d]' %(vertical, horizontal))
                movement()
            else:
